function lineGraph(country){
    ...
    fetch(`../CountryData/cases_per_day_agg_${country}.csv`)
            ...
            var columns = ['Confirmed', 'Active', 'Recovered', 'Deaths', "Click on each to filter lines"]
                ...
                mouseG.selectAll('#lineNode')
                    ...
                    .attr('y', function(d,i){
                        ...
                        console.log("RSKLOG - "+d+i)
                        legendList = {"Confirmed":0,"Active":1, "Recovered":2, "Deaths":3}
                        if(d3.selectAll(line_classes[legendList[i]]).attr('visibility') == 'hidden'){
                            d3.selectAll(line_classes[legendList[i]]).attr('visibility', 'visible')
                            d3.selectAll('#mouseLine'+legendList[i]).attr('visibility','visible')
                            
                        }
                        else{
                            d3.selectAll(line_classes[legendList[i]]).attr('visibility', 'hidden')
                            d3.selectAll('#mouseLine'+legendList[i]).attr('visibility','hidden')

                        }
                    })
                    .on('mouseover', function(d,i){
                        legendList = {"Confirmed":0,"Active":1, "Recovered":2, "Deaths":3}
                        d3.selectAll(line_classes[legendList[i]]).style('opacity',0.4)
                        d3.select(this).style('cursor', 'pointer')
                    })
                    .on('mouseout', function(d,i){
                        legendList = {"Confirmed":0,"Active":1, "Recovered":2, "Deaths":3}
                        d3.selectAll(line_classes[legendList[i]]).style('opacity',1)
                    });
